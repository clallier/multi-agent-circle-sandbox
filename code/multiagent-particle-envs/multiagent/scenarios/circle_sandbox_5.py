...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for goal in goals:
            ...
            goal.activate = False
            goal.activate_time = randrange(10, 15) * (i+1)
    ...
    # def estimate_target_pos(self, agent, target, coef=2.):
    #     p_vel_mag = dist(np.array([0, 0]), target.state.p_vel)
    ...
    def reward(self, agent, world):
        ...
        if landmark and landmark.activate:
            a_dist = dist(agent.state.p_pos, landmark.state.p_pos)
        
        # else follow the leader
        else:
            target_pos = self.estimate_target_pos(agent, target)
            a_dist = dist(agent.state.p_pos, target_pos)
            
        
        # reward = .1 * min(1/a_dist, 10)
